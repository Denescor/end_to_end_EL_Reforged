...
if not paths.filep(rewtr_t7filename) then
  ...
  local files = {'TR_train.csv', 'TR_test.csv'}--, 'aida_train.csv', 'aida_testA.csv', 'aida_testB.csv', 
  ...
  for _,f in pairs(files) do
    ...
    while line do
      local parts = split(line, '\t')
      --print("##############\n" .. line .. "\n##############")
      if table_len(parts) >= 6 and parts[6] == 'CANDIDATES' and parts[table_len(parts) - 1] == 'GT:' then
          --if parts[6] ~= 'CANDIDATES' then
          --  print('len parts : ' .. #parts)
          --  print(parts)
          --  assert(parts[6] == 'CANDIDATES')
          --end
          --if parts[table_len(parts) - 1] ~= 'GT:' then
          --  print('len parts : ' .. #parts)
          --  print(parts)
  ...
  print("lines not passed : " .. nb_pass .. " / " .. total_pass)
