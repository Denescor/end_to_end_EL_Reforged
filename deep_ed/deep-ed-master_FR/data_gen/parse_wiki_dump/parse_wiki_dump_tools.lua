...
function extract_text_and_hyp(line, mark_mentions)
  ...
  while begin_start_hyp do
    text = text ..line:sub(end_end_hyp + 1, begin_start_hyp - 1)  -- utf8sub(line, end_end_hyp + 1, begin_start_hyp - 1)
    
    local next_quotes,end_quotes = line:find('">', end_start_hyp + 1)
    if next_quotes then
      local ent_name = line:sub(end_start_hyp + 1, next_quotes - 1) -- utf8sub(line, end_start_hyp + 1, next_quotes - 1)
      begin_end_hyp, end_end_hyp = line:find('</a>', end_quotes + 1)
      if begin_end_hyp then
        local mention = line:sub(end_quotes + 1, begin_end_hyp - 1) -- utf8sub(line, end_quotes + 1, begin_end_hyp - 1)
          ...
          if (not i) or (i ~= 1) then
            if ent_name:find('#') then
              ...
              if ent_wikiid == unk_ent_wikiid then
                ...
                text_disam = text_disam .. ent_name .. " || "
              else
                -- A valid (entity,mention) pair
                text_ok = text_ok .. ent_name .. "||"
  ...
  if end_end_hyp then
    --TODO error with line:len(). But "text" is never used so we can just ignore the bug
    ar1 = end_end_hyp + 1
    ar2 = len_line - 1
    text = text .. line:sub(end_end_hyp + 1) --.. utf8sub(line, ar1, ar2) -- 
  else
  ...
  return list_hyp, text, list_ent_errors, parsing_errors, disambiguation_ent_errors, diez_ent_errors, text_ok, text_disam
end
...
function extract_page_entity_title(line)
  ...
  local ent_wikiid = tonumber(line:sub(endoff + 1, startquotes - 1))--utf8sub(line,endoff + 1, startquotes - 1)
  assert(ent_wikiid, line .. ' ==> ' .. line:sub(startoff + 1, startquotes - 1))--utf8sub(line,startoff + 1, startquotes - 1)
  local starttitlestartoff, starttitleendoff = line:find(' title="')
  local endtitleoff, _ = line:find('">')
  local ent_name = line:sub(starttitleendoff + 1, endtitleoff - 1) --utf8sub(line,starttitleendoff + 1, endtitleoff - 1)
