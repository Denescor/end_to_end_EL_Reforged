...
def train():
    ...
    name_log_scores = "./log_exp/{}_{}.txt".format(args.experiment_name, (((args.training_name.split("/"))[-1]).split("_"))[0]  )
    ...
    with model.sess as sess:
        ...
        # Mettre "args.eval_cnt < [n]" pour effectuer [n] epoch
            ...
            if args.eval_cnt%10 == 0 or args.eval_cnt == 1:
                # On sauve les embeddings toutes les 10 epochs.
                ...
                for index_tensor in range(len(projectors)):
                    index_batch = 0
                    for tensor in projectors[index_tensor]:
                        ts = np.shape(tensor)
        ...
        print_score_log(log_score, el_global_scores)
