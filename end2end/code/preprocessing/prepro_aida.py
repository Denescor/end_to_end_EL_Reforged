    ...
    with open(in_filepath) as fin, open(out_filepath, "w") as fout:
        ...
        for line in fin:
            ...
            if line.startswith("-DOCSTART-"):
                ...
                wiki_title = l[4]
                if args.unify: wiki_title = unify_entity_name(wiki_title[len("http://en.wikipedia.org/wiki/"):])
                else: wiki_title = wiki_title[len("http://en.wikipedia.org/wiki/"):].replace('_', ' ')
