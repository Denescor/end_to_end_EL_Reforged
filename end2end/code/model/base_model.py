...
class BaseModel(object):
    ...
    def reinitialize_weights(self, scope_name):
        ...
        variables = tf.get_variable(scope_name)
    ...
    def restore_session(self, option="latest"):
        ...
        self.ed_saver = tf.train.Saver(var_list=self.checkpoint_variables(), max_to_keep=self.args.checkpoints_num, reshape=True)
        self.el_saver = tf.train.Saver(var_list=self.checkpoint_variables(), max_to_keep=self.args.checkpoints_num, reshape=True)
    ...
    def save_weight_model(self, args, model, sess, input_handle_ph, handle, path):
        layers = sess.run(model.layers) #, feed_dict={input_handle_ph: handle, model.dropout: args.dropout, model.lr: model.args.lr})
        print("saving model... ",end="")
        try: pickle.dump(layers, open(path+"final_model.pkl", "wb"))
        except Exception as e: print(" model not saved :\n'{}'".format(e))
        else: print(" model save at '{}'".format(path))
        
    def load_pretrained_model(self, path):
        self.initialize_session()
        sess = self.sess
        test = "global_voting/psi_and_global_ffnn/output_bias:0"
        not_to_load = ["chars/variable:0"]
        try: 
            pretrained_layers = pickle.load(open(path, "rb"))
            for name, vector in pretrained_layers.items():
                print("{} :\t\t {} ({})".format(name, type(vector), np.shape(vector)))
            for current_layer in tf.trainable_variables():
                try:
                    name = current_layer.name
                    #if name in not_to_load: 
                    #    print("{} not loaded. Continue reloaded".format(name))
                    #    continue #on ne load pas ce noeud là
                    if name == test: print("------ Assign Verification : ------\nbefore assign : {}\nnumpy values : {}".format(current_layer.eval(session=sess),pretrained_layers[name]))
                    op_assign = current_layer.assign(pretrained_layers[name])
                    sess.run(op_assign)
                    if name == test: print("after assign : {}\n------------------------------".format(current_layer.eval(session=sess)))
                except: print("{} not loaded. Continue reloaded".format(name))
        except Exception as e: print("!!! model not reloaded !!!:\n'{}'".format(e))
        else: print("\n!!! model correctly reload !!!\n")
        self.init_embeddings()
