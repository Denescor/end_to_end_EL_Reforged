...
from model.model import Model #model.model_ablation
...
def validation_loss_calculation(model, iterator, dataset_handle, opt_thr, el_mode, name=""):
    if args.print_predictions:
        print("writing print predictions")
...
def evaluate():
    ...
    for el_mode, datasets, names in zip([True, False], [el_datasets, ed_datasets], [el_names, ed_names]):
        if names == []:
            continue
        ...
        with model.sess as sess:
            ...
            f1 = compute_ed_el_scores(model, handles, names, iterators, el_mode=el_mode)
            print("{} : Macro F1 = {}".format(names,f1))
...
if __name__ == "__main__":
    args, train_args = _process_args(_parse_args())
    print("train_args:\n", train_args)
    ...
    train.args = args
    args.context_bert_lstm = train_args.context_bert_lstm
    args.context_bert = train_args.context_bert
    args.word_bert = train_args.word_bert
