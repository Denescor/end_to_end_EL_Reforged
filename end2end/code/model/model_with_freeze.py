...
import model.transformers_model as Transformer
...
class Model(BaseModel):
        ...
        with tf.variable_scope("chars"): #{}_chars".format(self.name)):
            if self.args.use_chars:
                ...
                _char_embeddings = tf.get_variable(
                        name="variable",
                        dtype=tf.float32,
                        shape=[self.nchars, self.args.dim_char], trainable=self.args.notfreeze)
                ...
                # bi lstm on chars
                cell_fw = tf.nn.rnn_cell.LSTMCell(self.args.hidden_size_char, state_is_tuple=True, trainable=self.args.notfreeze) #tf.contrib.rnn.LSTMCell(self.args.hidden_size_char, state_is_tuple=True)
                cell_bw = tf.nn.rnn_cell.LSTMCell(self.args.hidden_size_char, state_is_tuple=True, trainable=self.args.notfreeze) #tf.contrib.rnn.LSTMCell(self.args.hidden_size_char, state_is_tuple=True)
    ...
    def add_context_emb_op(self):
        ...
        with tf.variable_scope("context-bi-lstm"): #{}_context-bi-lstm".format(self.name)):
            ...
            if(("transformer" in dictargs) and (self.args.transformer)):
                ...
                cell_fw = tf.nn.rnn_cell.LSTMCell(self.args.hidden_size_lstm, trainable=self.args.notfreeze) #tf.contrib.rnn.LSTMCell(self.args.hidden_size_lstm)
                cell_bw = tf.nn.rnn_cell.LSTMCell(self.args.hidden_size_lstm, trainable=self.args.notfreeze) #tf.contrib.rnn.LSTMCell(self.args.hidden_size_lstm)
    ...
    def build(self):
        ...
        print(50*"/"+"\nTrainable var_name_list (self.layers) :")
        for v in var_name_list:
            print("- {}".format(v))
        print(50*"/")
        
        var_name_list_2 = [v.name for v in tf.get_collection(tf.GraphKeys.VARIABLES) if v not in var_name_list]
        print("Non Trainable var_name_list (self.layers) :")
        for v in var_name_list_2:
            print("- {}".format(v))
        print(50*"/")
