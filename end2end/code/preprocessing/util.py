...
def load_wikiid2nnid(extension_name=None, txt_file="wikiid2nnid.txt", verbose=True):
    ...
    with open(config.base_folder+"data/entities/wikiid2nnid/"+txt_file) as fin:
        for line in fin:
            ent_id, nnid = line.split('\t')
            wikiid2nnid[ent_id] = int(nnid) - 1  # torch starts from 1 instead of zero
        if "1" in wikiid2nnid and wikiid2nnid["1"] == 0: 
            del wikiid2nnid["1"]
            wikiid2nnid["<u>"] = 0
    ...
    if extension_name:
        load_entity_extension(wikiid2nnid, extension_name, verbose=verbose)
    return wikiid2nnid


def load_entity_extension(wikiid2nnid, extension_name, verbose=True):
    ...
    with open(filepath) as fin:
        ...
        for line in fin:
            ...
            line_cnt += 1
    if verbose : print("original entities: ", max_nnid + 1, " extension entities: ", len(wikiid2nnid) - (max_nnid+1))
...
def load_redirections(lowercase=None, verbose=True):
...
def load_disambiguations(verbose=True):
    wall_start = time.time()
...
def load_persons(verbose=True, wiki_path="wiki_name_id_map.txt"):
    wiki_name_id_map, _ = load_wiki_name_id_map(verbose=verbose, filepath=wiki_path)
    persons_wikiids = set()
    not_found_cnt = 0
    ...
    if verbose : print("persons not_found_cnt:", not_found_cnt)
    return persons_wikiids

def load_wiki_name_id_map(lowercase=False, filepath=None, verbose=True):
    wall_start = time.time()
    ...
    folder = config.base_folder + "data/basic_data/"
    if filepath is None:
        filepath = "wiki_name_id_map.txt"
    disambiguations_ids = load_disambiguations(verbose=verbose)
...
class FetchCandidateEntities(object):
    ...
    def __init__(self, args):
        ...
        self.p_e_m, self.p_e_m_low, self.mention_total_freq = custom_p_e_m(
            ...
            wiki_path=args.wiki_id_file,
            prob_file=args.prob_p_e_m)
...
class FetchFilteredCoreferencedCandEntities(object):
    def __init__(self, args):
        ...
        if args.persons_coreference:
            self.persons_wikiids = load_persons(wiki_path=args.wiki_id_file)
...
class EntityNameIdMap(object):
    ...
    def init_compatible_ent_id(self, wiki_map_file=None):
        self.wiki_name_id_map, self.wiki_id_name_map = load_wiki_name_id_map(lowercase=False, filepath=wiki_map_file, verbose=False)

    def init_gerbil_compatible_ent_id(self, wiki_map_file=None):
        self.wiki_name_id_map, self.wiki_id_name_map = load_wiki_name_id_map(lowercase=False, filepath=wiki_map_file, verbose=False)
        self.redirections = load_redirections(lowercase=False)

    def init_hyperlink2id(self, wiki_map_file=None):
        self.wiki_name_id_map, self.wiki_id_name_map = load_wiki_name_id_map(lowercase=False, filepath=wiki_map_file, verbose=False)
        self.wiki_name_id_map_l, _ = load_wiki_name_id_map(lowercase=True, filepath=wiki_map_file, verbose=False)
        self.redirections = load_redirections(lowercase=False)
...
def custom_p_e_m(cand_ent_num=15, allowed_entities_set=None,
                 ...
                 wiki_path="wiki_name_id_map.txt",prob_file="prob_yago_crosswikis_wikipedia_p_e_m.txt"):
    """Args:
    ...
    entityNameIdMap.init_compatible_ent_id(wiki_map_file=wiki_path)
    incompatible_ent_ids = 0
    with open(config.base_folder + 'data/basic_data/'+prob_file) as fin:
