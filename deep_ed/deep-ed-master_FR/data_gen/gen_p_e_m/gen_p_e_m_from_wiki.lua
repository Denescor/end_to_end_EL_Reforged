...
while (line) do
  ...
  if not line:find('<doc id="') then
    list_hyp, text, le_errs, p_errs, dis_errs, diez_errs, text_ok, text_disam = extract_text_and_hyp(line, false)
    ...
    if num_lines % 5000000 == 0 then
      print("ent valid : " .. text_ok)
      print("ent disambig errors : " .. text_disam)
    end
...
print('Processed all lines (' .. num_lines .. '). Parsing errs = ' .. 
          parsing_errors .. ' List ent errs = ' .. 
          list_ent_errors .. ' diez errs = ' .. diez_ent_errors .. 
          ' disambig errs = ' .. disambiguation_ent_errors .. 
          ' . Num valid hyperlinks = ' .. num_valid_hyperlinks)

total = parsing_errors+list_ent_errors+diez_ent_errors+disambiguation_ent_errors+num_valid_hyperlinks
prc_correct = 100 * (num_valid_hyperlinks/total)

print('    Done computing Wikipedia p(e|m). Num valid hyperlinks = ' .. num_valid_hyperlinks .. ' (' .. prc_correct .. '%)')
